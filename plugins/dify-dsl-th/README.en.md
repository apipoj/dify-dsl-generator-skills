...
Install directly from the GitHub marketplace. No manual clone or build is needed; `dify-dsl-th` includes all 12 skills and their supporting files.
...
Start a fresh session and invoke `/dify-dsl-th:using-dify-dsl` followed by your workflow request.

From a terminal, use `claude plugin marketplace add apipoj/dify-dsl-generator-skills` and `claude plugin install dify-dsl-th@apipoj-dify`.

### Codex

Using a Codex version with plugin support:
...
The marketplace is named `apipoj-dify`; the plugin is `dify-dsl-th`. Running DSL validation scripts requires Python 3.10+ and Ruby. Choose either plugins or the older symlink installation to avoid duplicate skills. See the [plugin guide](docs/plugins.th.md) for updates, migration from `personal`, and developer builds.
